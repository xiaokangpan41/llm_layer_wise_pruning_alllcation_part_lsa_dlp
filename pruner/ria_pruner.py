    ...
    for i in range(len(layers)):
        ...
        for name in subset:
            ...
            W_metric = (torch.abs(W)/torch.sum(torch.abs(W), dim=0) +
                        torch.abs(W)/torch.sum(torch.abs(W), dim=1).reshape(-1, 1)) * (torch.sqrt(X)) ** 0.5
            ...
            if prune_n != 0:
                ...
                if args.use_variant:
                    ...
                    while (torch.abs(cur_sparsity - args.final_s) > 0.001) and (
                            alpha_hist[1] - alpha_hist[0] >= 0.001):
                        if cur_sparsity > args.final_s:
                    ...
                    indices = sort_res[1][:, :int(W_metric.shape[1] * args.final_s)]
...
class RIAPruner:
    ...
    def prune(self, args):
        ...
        prune_ria(args, self.model, self.tokenizer, device=self.model.device, prune_n=args.N, prune_m=args.M)
