...
def owl_score(args, model, tokenizer, A, device=torch.device("cuda:0")):
    use_cache = model.config.use_cache
    model.config.use_cache = False
    ...
    # if opt_flag:
    #     N = 8
    #     mean_ratio = torch.mean(all_layer_ratio[N:])
    #     all_layer_ratio[:N] = mean_ratio

    all_layer_ratio = ((all_layer_ratio - all_layer_ratio.min()) * (
            1 / (all_layer_ratio.max() - all_layer_ratio.min()) * args.Lamda * 2))

    all_layer_ratio = args.final_s - all_layer_ratio + torch.mean(all_layer_ratio)
...
def owl_score_global(args, model, tokenizer, A, device=torch.device("cuda:0")):
    use_cache = model.config.use_cache
    model.config.use_cache = False
    ...
    for i in range(len(layers)):
        ...
        if args.layer == "owlb":
            ...
            attn_ratio = check_outlier_mean(layer_wmetric[:attn_numel], A)
            ffn_ratio = check_outlier_mean(layer_wmetric[attn_numel:], A)
            all_layer_metric.append([attn_ratio] * 4 + [ffn_ratio] * (len(layer_numel) - 4))
        else:
            ratios = [check_outlier_mean(x.cpu(), A) for x in layer_wmetric]
            all_layer_metric.append(ratios)

    layer_imp = torch.tensor(all_layer_metric)
...
class OWL:
    ...
    def get_layer_sp(self, args):
        if args.layer == "owl":
            return owl_score(args, self.model, self.tokenizer, self.A, device=self.model.device)
        elif args.layer == "owlb" or args.layer == "owlc":
            return owl_score_global(args, self.model, self.tokenizer, self.A,
