...
class QuantLinear(nn.Module):
    ...
    def pack(self, linear, scales, zeros, g_idx=None):
        ...
        intweight = intweight.to(torch.int32)  # 转换为 int32 类型
        qweight = torch.zeros(
        ...
        zeros -= 1
        zeros = zeros.to(torch.int32)  # 转换为 int32 类型
        qzeros = torch.zeros(
        ...
        self.qzeros = qzeros
        self.wf = self.wf.to(zeros.device)
    ...
    def forward(self, x):
        ...
        if self.is_cuda is True and (
            ...
            if self.bits == 2:
                ...
                quant_cuda.vecquant16matmul(x.float(), self.qweight, out, self.scales.float(), self.qzeros, self.g_idx)
            out = out.half()
        else:
            if self.bits in [2, 4, 8, 16]:
                zeros = torch.bitwise_right_shift(torch.unsqueeze(self.qzeros, 2).expand(-1, -1, 32 // self.bits),
                                                  self.wf.unsqueeze(0)).to(
                    torch.int16 if self.bits == 8 or self.bits == 16 else torch.int8)
