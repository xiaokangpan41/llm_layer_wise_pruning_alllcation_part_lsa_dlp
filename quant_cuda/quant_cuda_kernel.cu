...
template <typename scalar_t>
__global__ void VecQuant16MatMulKernel(
    const  scalar_t* __restrict__ vec,
    const       int* __restrict__ mat,
           scalar_t* __restrict__ mul,
    const  scalar_t* __restrict__ scales,
...
void vecquant16matmul_cuda(
  ...
  dim3 blocks(
    (height + BLOCKHEIGHT16 - 1) / BLOCKHEIGHT16,
    (width + BLOCKWIDTH - 1) / BLOCKWIDTH
  );
  dim3 threads(BLOCKWIDTH);

  AT_DISPATCH_FLOATING_TYPES(
    vec.type(), "vecquant16matmul_cuda", ([&] {
      VecQuant16MatMulKernel<<<blocks, threads>>>(
        vec.data<scalar_t>(), mat.data<int>(), mul.data<scalar_t>(),
        scales.data<scalar_t>(), zeros.data<int>(), g_idx.data<int>(),
        batch, vec_height, height, width, zero_width
      );
...
__global__ void VecQuant16MatMulKernel(
  ...
  __shared__ scalar_t blockvec[BLOCKWIDTH];
  int i = width * h + w;
  int g_h = h * 2;  // 每个32位整数包含2个16位权重
  int k;
  unsigned int g;
  scalar_t w_tmp;

  int z_w = w / 2;  // 每个32位整数包含2个16位零点值
  int z_mod = (w % 2) * 16;  // 16位偏移

  float weight[BLOCKWIDTH];

  for (k = 0; k < BLOCKWIDTH; ++k){
    int k_w = (k / 2);  // 每个32位整数包含2个16位权重
    int k_bit = (k % 2) * 16;  // 16位偏移

    g = as_int(g_idx[g_h + k]);
    scalar_t scale = scales[g * width + w];
    scalar_t zero = scalar_t(((as_unsigned(zeros[g * zero_width + z_w]) >> z_mod) & 0xFFFF) + 1);

    w_tmp = ((as_unsigned(mat[i + (k_w * width)]) >> k_bit) & 0xFFFF);
...
// __device__ inline half precise_quantize(float val, float scale, half zero_point) {
//     // 使用更精确的四舍五入方式
//     float quantized = roundf(val / scale) + __half2float(zero_point);
//     // 确保结果在 half 的取值范围内
//     return __float2half(fminf(65504.0f, fmaxf(-65504.0f, quantized)));
// }
//
// template <typename scalar_t>
// __global__ void VecQuant16MatMulKernel(
...
//
//     if (h < height && w < width) {
//         int group = group_idx[h * width + w];
//         float scale = scales[group];
//         int16_t zero = zero_points[group];
//
//         // 使用更精确的量化函数
//         output[h * width + w] = static_cast<int16_t>(precise_quantize(input[h * width + w], scale, zero));
//     }
// }
//
// void vecquant16matmul_cuda(
...
//     int zero_width = zeros.size(1);
//
//     dim3 blocks((height + 16 - 1) / 16, (width + 16 - 1) / 16);
//     dim3 threads(16, 16);
//
//     AT_DISPATCH_FLOATING_TYPES(vec.type(), "vecquant16matmul_cuda", ([&] {
//         VecQuant16MatMulKernel<<<blocks, threads>>>(
