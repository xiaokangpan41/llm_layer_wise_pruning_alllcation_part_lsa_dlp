...
# 在8bit测试后添加16bit测试
mat = torch.randint(-1000000000, 1000000000, (M // 32 * 16, N), device=DEV, dtype=torch.int)
scales = torch.randn(N, device=DEV, dtype=DTYPE)
zeros = torch.randint(-1000000000, 1000000000,(1,N // 32 * 16), device=DEV,dtype=torch.int32)

vec = vec.float()
tick = time.time()
for _ in range(COUNT):
    quant_cuda.vecquant16matmul(vec, mat, mul, scales, zeros, g_idx)
    torch.cuda.synchronize()
print('16bit:', (time.time() - tick) / COUNT)
...
qlayer = QuantLinear(2, -1, layer.in_features, layer.out_features, layer.bias is not None,
                     kernel_switch_threshold=False, is_cuda=True)
...
qlayer = QuantLinear(3, -1, layer.in_features, layer.out_features, layer.bias is not None,
                     kernel_switch_threshold=False, is_cuda=True)
qlayer.pack(layer, quantizer.scale, quantizer.zero)
...
qlayer = QuantLinear(4, -1, layer.in_features, layer.out_features, layer.bias is not None,
                     kernel_switch_threshold=False, is_cuda=True)
qlayer.pack(layer, quantizer.scale, quantizer.zero)
...
qlayer = QuantLinear(8, -1, layer.in_features, layer.out_features, layer.bias is not None,
                     kernel_switch_threshold=False, is_cuda=True)
qlayer.pack(layer, quantizer.scale, quantizer.zero)
...
quantizer = Quantizer()
quantizer.configure(16, perchannel=True, sym=False, mse=False)
quantizer.find_params(layer.weight.data, weight=True)
layer.weight.data = quantize(layer.weight.data, quantizer.scale, quantizer.zero, quantizer.maxq)

qlayer = QuantLinear(16, -1, layer.in_features, layer.out_features, layer.bias is not None,kernel_switch_threshold = False, is_cuda = True)
qlayer.pack(layer, quantizer.scale, quantizer.zero)
...
with torch.no_grad():
    print('16bit Simu:', layer(vec))
    print('16bit Kern:', qlayer(vec))